...
These files are not available and are not part of the deposit. The consequence
is stated in the paper: the *N* = 1–5 series archived in
`data/processed/scaling_results.csv` cannot be regenerated by re-executing the
physics, so it is an input to the analysis rather than a reproducible output of
it.

Two things bear on how much that matters. `docs/SIMULATION_SPEC.md`
